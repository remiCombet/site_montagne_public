...
const StayDetails = ({ stay, onUpdate, className, onMessage }) => {
    ...
    const handleSubmitDetails = async (e) => {
        ...
        if (!stay?.id) {
            onMessage({ type: "error", text: "L'ID du séjour est manquant" });
            return;
        }
        
        if (!userId) {
            onMessage({ type: "error", text: "Vous devez être connecté pour effectuer cette action" });
            return;
        }
        ...
        const updatedStay = {
            ...
            reception_point_id: stay.reception_point_id, // Utiliser directement celui du stay
            status,
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                onMessage({ type: "success", text: "Séjour mis à jour avec succès !" });
            ...
            onMessage({ type: "error", text: "Erreur lors de la mise à jour du séjour" });
        } finally {
    ...
    const handleDeleteStay = async () => {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                onMessage({ type: "success", text: "Séjour supprimé avec succès !" });
                ...
                onMessage({ type: "error", text: res.message || "Erreur lors de la suppression du séjour" });
            }
        } catch (error) {
            console.error("Erreur lors de la suppression du séjour:", error);
            onMessage({ type: "error", text: "Erreur lors de la suppression du séjour" });
        } finally {
            setIsSubmitting(false);
