...
const HighlightTest = ({ stay, onUpdate, onMessage }) => {
        ...
        getHighlightsByStayId(stayId)
            .then((res) => {
                if (res.status === 200) {
                    ...
                    onMessage({ type: 'error', text: 'Erreur lors du chargement des points forts' });
                }
            })
            .catch((err) => {
                console.error('Erreur lors du chargement des points forts:', err);
                onMessage({ type: 'error', text: 'Impossible de récupérer les points forts du séjour' });
            });
    }, [stayId]);
    ...
    const handleSubmit = (e) => {
        ...
        if (validationErrors.length > 0) {
            onMessage({ type: 'error', text: validationErrors.join(', ') });
            ...
            updateHighlight(editingHighlightId, updatedHighlight)
                .then((res) => {
                    if (res.status === 200) {
                        ...
                        onMessage({ type: 'success', text: 'Point fort modifié avec succès' });
                        ...
                        onMessage({ type: 'error', text: res.msg || 'Erreur lors de la mise à jour' });
                    }
                })
                .catch((err) => {
                    console.error('Erreur lors de la mise à jour:', err);
                    onMessage({ type: 'error', text: 'Erreur lors de la mise à jour du point fort' });
                })
                .finally(() => {
                    setIsSubmitting(false);
                });
            ...
            createHighlight(newHighlight)
                .then((res) => {
                    if (res.status === 200) {
                        ...
                        onMessage({ type: 'success', text: 'Point fort ajouté avec succès' });
                ...
                .catch((err) => {
                    ...
                    onMessage({ type: 'error', text: 'Erreur lors de la création du point fort' });
                })
                .finally(() => {
                    setIsSubmitting(false);
                });
        ...
        deleteHighlight(highlightId)
            .then((res) => {
                if (res.status === 200) {
                    ...
                    onMessage({ type: 'success', text: 'Point fort supprimé avec succès' });
                    ...
                    onMessage({ type: 'error', text: res.msg || 'Erreur lors de la suppression' });
                }
            })
            .catch((err) => {
                console.error('Erreur lors de la suppression:', err);
                onMessage({ type: 'error', text: 'Erreur lors de la suppression du point fort' });
            })
            .finally(() => {
                setIsSubmitting(false);
            });
