...
const EquipmentsTest = ({ stay, onUpdate, onMessage }) => {
    ...
    const loadData = () => {
        ...
        .catch(err => {
            ...
            onMessage({ type: "error", text: "Erreur lors du chargement des données" });
        })
        .finally(() => {
    ...
    const loadEquipments = () => {
        ...
        .catch(err => {
            ...
            onMessage({ type: "error", text: "Erreur lors du rechargement des équipements" });
        })
        .finally(() => {
            setIsSubmitting(false);
        });
    ...
    const handleAddEquipment = (categoryId, type) => {
        if (!stay?.id || !categoryId) {
            onMessage({ type: "error", text: "Paramètres invalides" });
            return;
        }
        ...
        apiCall
            ...
            .catch(err => {
                ...
                onMessage({ type: "error", text: "Erreur lors de l'ajout de l'équipement" });
            })
            .finally(() => {
                setIsSubmitting(false);
    ...
    const handleCreateCategory = async (e) => {
        ...
        try {
            ...
            if (res.status === 200) {
                onMessage({ type: "success", text: "Catégorie créée avec succès" });
                ...
                onMessage({ type: "error", text: res.msg || "Erreur lors de la création" });
            }
        } catch (err) {
            console.error('Erreur création catégorie:', err);
            onMessage({ type: "error", text: "Erreur lors de la création de la catégorie" });
        } finally {
            setIsSubmitting(false);
        }
    };
        ...
        return (
            <section className="equipment-view-mode">
                <header className="equipment-header">
                    ...
                    <nav className="equipment-tabs" role="tablist">
                        <button 
                            type="button"
                            role="tab"
                            ...
                            aria-selected={activeTab === 'provided'}
                            aria-controls="provided-panel"
                            id="provided-tab"
                        >
                            Équipements fournis
                        </button>
                        <button 
                            ...
                            aria-selected={activeTab === 'toPrepare'}
                            aria-controls="toPrepare-panel"
                            id="toPrepare-tab"
                        >
                                            ...
                                            <ul className="equipment-items">
                                                {group.items.map((item, i) => (
                                                    <li key={`provided-item-${i}`} className="equipment-item">
                                                        <span className="equipment-name">{decodeHTML(item.category.name)}</span>
                                                        <span className="equipment-desc">{decodeHTML(item.category.description)}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        </li>
            ...
            <section className="equipment-edit-mode">
                                    ...
                                    >
                                        ...
                                        {availableCategories.map(category => (
                                            <option key={category.id} value={category.id}>
                                                {category.type.toUpperCase()} - {decodeHTML(category.name)}: {decodeHTML(category.description)}
                                            </option>
                                        ))}
                                    </select>
                                    <button 
                ...
                <section className="equipment-form-section">
                    <header className="form-title-container">
                        <h4 id="form-title" className="form-title">
